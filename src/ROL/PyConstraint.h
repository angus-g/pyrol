...
class PyConstraint : public ROL::Constraint<double> {
 public:
  void value(ROL::Vector<double> &c, const ROL::Vector<double> &x,
    ...
    if (overload)
      return overload.operator()<py::return_value_policy::reference>(c, x, tol)
          .cast<void>();
    else
  ...
  virtual void applyJacobian(ROL::Vector<double> &jv,
    ...
    if (overload)
      return overload
          .
          operator()<py::return_value_policy::reference>(jv, v, x, tol)
          .cast<void>();
    else
  ...
  virtual void applyAdjointJacobian(ROL::Vector<double> &ajv,
    ...
    pybind11::function overload =
        py::get_overload(this, "applyAdjointJacobian");
    if (overload)
      return overload
          .
          operator()<py::return_value_policy::reference>(ajv, v, x, tol)
          .cast<void>();
    else
  ...
  virtual void update(const ROL::Vector<double> &x, bool flag = true,
    ...
    if (overload)
      return overload
          .
          operator()<py::return_value_policy::reference>(x, flag, iter)
          .cast<void>();
    else
