...
class CustomLA : public std::enable_shared_from_this<CustomLA>,
                 public ROL::Vector<double> {
 public:
  CustomLA() {}
  ...
  virtual void scale(const double alpha) {
    ...
    if (overload)
      return overload.operator()<py::return_value_policy::reference>(alpha)
          .cast<void>();
    else
  ...
  virtual int dimension() const {
    ...
    if (overload)
      return overload.operator()<py::return_value_policy::reference>()
          .cast<int>();
    else
  ...
  virtual std::shared_ptr<ROL::Vector<double>> clone() const {
    ...
    if (overload) {
      ...
      // A wiser man than me maybe knows why this ref count increase is
      // necessary...
      Py_INCREF(res.ptr());
      return res.cast<std::shared_ptr<CustomLA>>();
    } else
  ...
  virtual double getitem(const int& i) const {
    ...
    if (overload)
      return overload.operator()<py::return_value_policy::reference>(i)
          .cast<double>();
    else
  ...
  virtual void setitem(const int& i, const double& val) const {
    ...
    if (overload)
      overload.operator()<py::return_value_policy::reference>(i, val)
          .cast<void>();
    else
